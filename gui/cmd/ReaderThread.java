...
public class ReaderThread implements Runnable {
    ...
    ReaderThread(PipedInputStream pinInput1, PipedInputStream pinInput2, Thread errorThrower1, Thread reader11,
            Thread reader22, boolean newflag, TextArea txtArea1) {
        pipeIn = pinInput1;
        pipeIn2 = pinInput2;
    ...
    public synchronized void run() {
        try {
            ...
            while (Thread.currentThread() == this.reader2) {
                ...
                if (this.quit)
                    return; // if some one closed the stage then this check will be performed every time, if
                            // true
