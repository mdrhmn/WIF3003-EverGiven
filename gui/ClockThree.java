 ...
 */
public class ClockThree extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        RotateTransition secondsTransition = createRotateTransition(Duration.seconds(60), secondHand,
                getSecondsAngle(time));
    ...
    Node createTickMarks() {
        ...
        List<Node> tickMarks = IntStream.range(0, 60).mapToObj(this::createTickMark).collect(toList());
    ...
    Node createHourOrMinuteHand(double stretchRelativeToRim, Color color) {
        Path path = new Path(new MoveTo(UNIT_SIZE, UNIT_SIZE), new LineTo(UNIT_SIZE * 0.9, UNIT_SIZE * 0.9),
                new LineTo(UNIT_SIZE, stretchRelativeToRim), new LineTo(UNIT_SIZE * 1.1, UNIT_SIZE * 0.9),
